...
const ProjectList: React.FC<ProjectListProps> = ({
  ...
  // Force close modal when list is not active
  React.useEffect(() => {
    if (!isActive && selectedProject !== null && !isMobile) {
      closeModal();
      setHoveredProjectIndex(null);
    }
  }, [isActive, selectedProject, isMobile, closeModal]);
  ...
  return (
    <div className="relative flex flex-col w-full h-screen items-center px-4 overflow-auto justify-center md:w-2/3 min-h-screen">
      ...
      <div
        ref={projectListRef}
        className={`space-y-2 w-full z-10 ${isMobile && selectedProject !== null ? "hidden" : ""
          }`}
        onMouseLeave={() => {
          if (!isMobile) {
            closeModal();
      ...
      {selectedProject !== null && (
        <motion.div
          ref={modalRef}
          className={`fixed w-full md:w-2/5 lg:w-2/4 h-auto rounded-2xl flex flex-col justify-center items-center md:right-12 p-6 z-20 overflow-hidden md:pointer-events-none ${theme === 'dark'
            ...
            } before:absolute before:inset-0 before:bg-gradient-to-br before:opacity-30 ${theme === 'dark'
        ...
        >
          <button
            className={`fixed top-16 md:top-6 right-3 rounded-full p-2 z-50 md:hidden transition-all duration-300 backdrop-blur-lg border ${theme === 'dark'
          ...
          <Suspense fallback={null}>
            <FadeContent blur={true} duration={1000} easing="ease-out" initialOpacity={0}>
              <div
                ...
                className={`relative ${projects[selectedProject]?.name === "Bang Abah"
                  ? "aspect-w-9 aspect-h-16"
                  : "aspect-w-16 aspect-h-9"
                  } w-full rounded-2xl overflow-hidden bg-gradient-to-br ${theme === 'dark'
                    ? 'from-white/5 to-white/10'
                    : 'from-black/5 to-black/10'
                  } backdrop-blur-sm border ${theme === 'dark' ? 'border-white/20' : 'border-black/20'
              ...
              <div className="flex flex-wrap justify-center mt-4 gap-4">
                {projects[selectedProject]?.techStack.map((tech, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <span className={`text-md ${theme === 'dark' ? 'text-gray-300' : 'text-gray-800'
                      }`}>
                      {tech.name}
                    </span>
                  </div>
